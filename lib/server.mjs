...
const server = http.createServer( function( req, res ) {
    ...
    // The queryObject is stringify and then parsed to JSON to get rid of [Object: null prototype]
    let queryToString = JSON.stringify( parsedUrl.query );
    const queryStringObject = JSON.parse( queryToString );
    ...
    req.on( 'end', function() {
        ...
        try {
            ...
            debug( error );
            server.processHandlerResponse( res, method, trimmedPath, 500, { 'Error' : 'An unknown has ocurred' }, contentType );
        }
    } );
