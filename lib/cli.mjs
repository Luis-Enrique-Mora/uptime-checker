...
// * List all compressed logs available
cli.responders.listLogs = ( ) => {
    // look for logs file names on the logs directory
    _logs.list( true, function( error, logsFileNames ) {
        // If there is not error and there are logs, proceed to print logs file names on console
        if( !error && logsFileNames && logsFileNames.length > 0 ) {
            cli.verticalSpace();
            logsFileNames.forEach( name => {
                if( name.indexOf( '-' ) > -1 ) {
                    console.log( name );
                    cli.verticalSpace();
                }
...
// * Show in console more info about the log specified
cli.responders.moreLogInfo = ( str ) => {
    // Splits the string when it finds the characters --, so we can take the value that we need
    var array = str.split( '--' );
    // Takes the second value after the -- characters
    var checkId = array[ 1 ].trim();
    // validates that the value have the log name format, 34 characters and be a string (id-datetime)
    checkId = typeof( checkId ) == 'string' && checkId.length == 34 ? checkId : false;
    // If check id is valid
    if( checkId ) {
        // read 
        _logs.decompress( checkId, function( error, checkData ) {
            if( !error && checkData ) {
                cli.verticalSpace();
                // Parsing string to object
                var strToObject = JSON.parse( checkData );
                console.dir( strToObject, {'colors' : true } );
                cli.verticalSpace();
            }
        } );
    }
